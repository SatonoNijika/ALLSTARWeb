<template>
  <div class="dsgbox">
    <div class="container">
      <div class="flex">
        <div class="right">
          <div class="dsgtext">
            <div class="dsgcontainer">
              <div class="box">
                <div class="type"><span>A-STAR的</span></div>
                <div class="tit">愿景</div>
                <div class="txt">
                  手术方案导航预览界面直观清晰地观察规划的针头插入路径与人体各器官的空间关系，
                  有效避免了医生复杂的空间转换思维。此外，医生可以通过“拖拽控制”功能轻松调整机器人系统的执行端与人体之间的空间关系。
                  这进一步促进了术中导航期间更准确的穿刺路径和超声探头在体表上的位置。
                </div>
                <div class="btn">
                  <a href="#">阅读更多</a>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="left">
          <video src="/public/videos/doc.mp4" autoplay loop muted></video>
        </div>
      </div>
      <div class="flex">
        <div class="left">
          <video src="/public/videos/ct.mp4" autoplay loop muted></video>
        </div>
        <div class="right">
          <div class="dsgtext">
            <div class="dsgcontainer">
              <div class="box">
                <div class="type"><span>C-STAR的</span></div>
                <div class="tit">UR10机械臂</div>
                <div class="txt">
                  手术方案导航预览界面直观清晰地观察规划的针头插入路径与人体各器官的空间关系，
                  有效避免了医生复杂的空间转换思维。此外，医生可以通过“拖拽控制”功能轻松调整机器人系统的执行端与人体之间的空间关系。
                  这进一步促进了术中导航期间更准确的穿刺路径和超声探头在体表上的位置。
                </div>
                <div class="btn">
                  <a href="#">阅读更多</a>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="flex">
        <div class="right">
          <div class="dsgtext">
            <div class="dsgcontainer">
              <div class="box">
                <div class="type"><span>A-STAR的</span></div>
                <div class="tit">FTK500光学仪器</div>
                <div class="txt">
                  手术方案导航预览界面直观清晰地观察规划的针头插入路径与人体各器官的空间关系，
                  有效避免了医生复杂的空间转换思维。此外，医生可以通过“拖拽控制”功能轻松调整机器人系统的执行端与人体之间的空间关系。
                  这进一步促进了术中导航期间更准确的穿刺路径和超声探头在体表上的位置。
                </div>
                <div class="btn">
                  <a href="#">阅读更多</a>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="left">
          <video src="/public/videos/foot.mp4" autoplay loop muted></video>
        </div>
      </div>
      <div class="flex">
        <div class="left">
          <video src="/public/videos/shoushu.mp4" autoplay loop muted></video>
        </div>
        <div class="right">
          <div class="dsgtext">
            <div class="dsgcontainer">
              <div class="box">
                <div class="type"><span>C-STAR的</span></div>
                <div class="tit">On传感器</div>
                <div class="txt">
                  手术方案导航预览界面直观清晰地观察规划的针头插入路径与人体各器官的空间关系，
                  有效避免了医生复杂的空间转换思维。此外，医生可以通过“拖拽控制”功能轻松调整机器人系统的执行端与人体之间的空间关系。
                  这进一步促进了术中导航期间更准确的穿刺路径和超声探头在体表上的位置。
                </div>
                <div class="btn">
                  <a href="#">阅读更多</a>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<style scoped>
.container {
  margin-left: 346px;
  margin-right: 346px;
}

.flex {
  margin-left: -128px;
  margin-right: -128px;
  padding: 120px 0;
  display: flex;
}

.left {
  flex: 0.7;
}

.left video {
  width: 100%;
  height: auto;
  border-radius: 20px;
}

.right {
  flex: 1;
  display: flex;
  align-items: center; /* 垂直居中 */
}

.dsgtext {
  padding: 0 127px;
}

.type {
  font-size: 64px;
  text-align: left;
}

.tit {
  font-size: 26px;
  text-align: left;
}

.txt {
  padding-top: 26px;
  text-align: left;
  font-size: 32px;
  line-height: 1.8; /* 设置段间距为行高的1.8倍 */
}

.btn {
  padding-top: 54px; /* div link有上48的padding */
  text-align: left;
}

.btn a {
  height: 125px;
  width: 64px;
  font-size: 32px;
  background-color: rgb(41, 73, 195);
  color: white;
  text-decoration: none;
  border-radius: 20px; /* 设置边角为圆润效果 */
  padding: 5px 24px;
}
</style>
